...
#[case(2, 2)]
#[case(5, 1)]
fn cluster_converges_when_a_ready_is_dropped(#[case] size: usize, #[case] quorum: usize) {
    // Arrange
    ...
    assert!(
        ...
        "size {size}, quorum {quorum}: cluster did not reach Bootstrapped after a dropped ready"
    );
}
