...
use faction_system_tests::transport::channels::channels_transport::ChannelsTransport;
...
fn send_between_two_peers_received() {
    // Arrange
    let mut transports = ChannelsTransport::new_mesh(&[0, 1]);
...
fn send_three_peer_all_delivered() {
    // Arrange
    let mut transports = ChannelsTransport::new_mesh(&[0, 1, 2]);
